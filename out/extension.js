...
function loadJson(fileName) {
    ...
    try {
        ...
        vscode.window.showErrorMessage(localize('vmxEditor.LoadError', "Failed to load {0}", fileName));
        return [];
    }
    //return JSON.parse(fs.readFileSync(filePath, 'utf8'));
}
const completionData = loadJson('completionData');
const vmxBlocks = loadJson('vmxBlocks');
function resolveLocalized(defaultText, key, ...args) {
    if (key)
        return localize(key, defaultText ?? '', ...args);
    return defaultText ?? '';
}
function isoTimestamp() {
    return new Date().toISOString();
}
function isVMXFile(uri) {
...
function activate(context) {
    ...
    const vmxSelector = { language: 'vmx', scheme: 'file' };
    ...
    const insertTimestampCmd = vscode.commands.registerCommand('vmxEditor.InsertTimeStamp', async () => {
        ...
        await editor.insertSnippet(new vscode.SnippetString(`${localize('vmxEditor.ModifiedOn', "# Modified on")} ${ts}\n`), editor.selection.active);
    });
    context.subscriptions.push(insertTimestampCmd);
    function loadVmxBlocks(context) {
        const lang = getLanguage();
    ...
    //export
    function registerInsertBlockCommand(context) {
    ...
    async function createBackupIfNeeded(doc) {
        try {
            if (doc.isUntitled)
                return;
            ...
            console.warn(localize('vmxEditor.BackupFailed', "Backup creation failed: {0}"), err);
        }
    }
    ...
    const completionProvider = vscode.languages.registerCompletionItemProvider(vmxSelector, {
...
function validateVMXDocument(document) {
    const diagnostics = [];
...
function validateLineType(key, value) {
    const entry = completionData.find(e => e.label === key);
    if (!entry || !entry.type) {
        return null;
    }
    if (entry.type === 'boolean' && !/^(TRUE|FALSE)$/i.test(value)) {
        return localize('vmxEditor.BooleanExpected', "Boolean value expected: TRUE or FALSE");
    }
    if (entry.type === 'number' && !/^\d+$/.test(value)) {
        return localize('vmxEditor.NumberExpected', "Numeric value expected");
    }
    if (entry.type === 'enum' && entry.enumValues && !entry.enumValues.includes(value)) {
        return localize('vmxEditor.ValueExpected', "Value expected: {0}", entry.enumValues.join(','));
    }
    return null;
}
function deactivate() {
